...
class AGI:
    ...
    def say_datetime(self, time, escape_digits=''):
        """agi.say_datetime(time, escape_digits='') --> digit
        Throws AGIException on channel failure
        """
        escape_digits = self._process_digit_list(escape_digits)
	#format = "\"Ad 'msg_de' B 'msg_de' Y 'digits/at' IMp\""
	#format = "\"AdBY 'digits/at' IMp\""
	format = "\"AdBY\""
        res = self.execute('say datetime', time, escape_digits, format)['result'][0]
        if res == '-1':
            raise AGIException('Channel falure on channel %s' % self.env.get('agi_channel','UNKNOWN'))
        elif res == '0':
            return ''
